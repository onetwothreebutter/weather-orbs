  ...
  Promise.all([getWeatherData(lat, lon)]).then(res => {
    ...
    for (let i = 0; i < 24; i++) {
      setTimeout(function timer() {
        renderTheWeather(darksky.hourly.data[i], lat, lon);
      }, i * 1000);
    }
...
function renderTheWeather(darksky, lat, lon) {
  //round to the nearest 10
  let { apparentTemperature : temperature } = darksky;
  let nearestTemps = {
  ...
  //   settings.precipIntensity = lerp(3.0, 3.5, progress);
